...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.net_s2t = network.define_G(opt.image_nc, 
                                        opt.image_nc, 
                                        opt.ngf, 
                                        opt.transform_layers, 
                                        opt.norm,
                                        opt.activation, 
                                        opt.trans_model_type, 
                                        opt.init_type, 
                                        opt.drop_rate,
                                        False, 
                                        opt.gpu_ids, 
        ...
        self.net_img2task = network.define_G(opt.image_nc, 
                                             opt.label_nc, 
                                             opt.ngf, 
                                             opt.task_layers, 
                                             opt.norm,
    ...
    def set_input(self, data):
        self.input = data
        self.img_source = data['img_source']
        self.img_target = data['img_target']

        if len(self.gpu_ids) > 0:
            self.img_source = self.img_source.cuda()
            self.img_target = self.img_target.cuda()

        self.image_paths = data['img_target_paths']
